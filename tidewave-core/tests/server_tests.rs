...
async fn test_copy_file() {
    ...
    let temp_dir = std::env::temp_dir().join(format!("copy_test_{}", uuid::Uuid::new_v4()));
    fs::create_dir_all(&temp_dir).expect("Failed to create temp dir");
    let from_path = temp_dir.join("source.txt");
    let to_path = temp_dir.join("dest.txt");
    fs::write(&from_path, "hello copy").expect("Failed to write file");

    let client = reqwest::Client::new();

    let response = client
        .post(format!("http://127.0.0.1:{}/copy", port))
        .json(&serde_json::json!({
    ...
    assert_eq!(fs::read_to_string(&to_path).unwrap(), "hello copy");
...
#[tokio::test]
async fn test_copy_file_requires_absolute_paths() {
    let (port, shutdown_tx) = start_test_server(vec![]).await;
