...
async fn test_rename_file() {
    ...
    let temp_dir = std::env::temp_dir().join(format!("rename_test_{}", uuid::Uuid::new_v4()));
    fs::create_dir_all(&temp_dir).expect("Failed to create temp dir");
    let from_path = temp_dir.join("original.txt");
    let to_path = temp_dir.join("renamed.txt");
    fs::write(&from_path, "rename me").expect("Failed to write file");

    let client = reqwest::Client::new();

    let response = client
        .post(format!("http://127.0.0.1:{}/rename", port))
        .json(&serde_json::json!({
            "from": from_path.to_str().unwrap(),
            "to": to_path.to_str().unwrap()
        }))
        .send()
        .await
        .unwrap();
    ...
    assert_eq!(fs::read_to_string(&to_path).unwrap(), "rename me");
...
#[tokio::test]
async fn test_rename_file_requires_absolute_paths() {
    let (port, shutdown_tx) = start_test_server(vec![]).await;
...
#[tokio::test]
async fn test_rename_file_nonexistent() {
    let (port, shutdown_tx) = start_test_server(vec![]).await;
    ...
    let response = client
        .post(format!("http://127.0.0.1:{}/rename", port))
        .json(&serde_json::json!({
            "from": "/nonexistent/path/that/does/not/exist.txt",
            "to": "/tmp/dest.txt"
        }))
        .send()
        .await
